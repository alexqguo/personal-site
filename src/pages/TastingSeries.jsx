  ...
  return (
    <section id={id} className="__tasting-post mb-8">
      ...
      {frontMatter.subtitle ? (
        <div className="text-lg italic text-slate-700">
          {frontMatter.subtitle}
        </div>
      ) : null}
...
const TOC = ({ postsBySeason }) => (
  <ul className="mb-8">
    {Object.keys(postsBySeason).map((seasonNumber) => (
      <TOCSeason
        key={`s-${seasonNumber}`}
        posts={postsBySeason[seasonNumber]}
      />
    ))}
  </ul>
);
      ...
      <ul>
        {episodes.map((episode) => (
          <li className="ml-4 text-sm" key={episode.id}>
            <a href={`#${episode.id}`}>
              {episode.season}.{episode.episode} {episode.frontMatter.title}
  ...
  return (
    <>
      ...
      <section className="mb-4 text-sm">
        ...
        <ul>
          <li>
            <strong className="text-yellow-400">Gold: </strong>Shana L, Dave S,
            Pinghao Q, Nancy H, Raheed A, JT A, Ben H, Gandherva GT, Andrew S,
            Alex V, Vikas G
          </li>
          <li>
            <strong className="text-gray-400">Silver: </strong>Ishan P, Alberto
            N, Sean P
          </li>
          <li>
            <strong className="text-orange-600">Bronze: </strong>Dillon C,
            Akshay N
          </li>
        </ul>
      </section>
